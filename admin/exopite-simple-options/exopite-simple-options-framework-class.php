...
//				$this->config = wp_parse_args( $this->config, $default );
		...
		public function get_value( $options, $field  ) {
			 ...
			 *
			 *	   if ( $unique == $this->quotation_sign && $field['id'] == 'your-field-id' ) {
			 *		   // do the magic ;)
			 *	   }
			 *
			 *	   return $value;
...
/**
 * Use:
 *  $options = ( function_exists( 'get_exopite_sof_option' ) ) ? get_exopite_sof_option( $this->quotation_sign ) : get_option( $this->quotation_sign );

 *  // OR
 *
 *  if ( function_exists( 'get_exopite_sof_option' ) ) {
 *      $options = get_exopite_sof_option( $this->quotation_sign );
 *  } else {
 *      $options = get_option( $this->quotation_sign );
 *  }
 */
if ( ! function_exists( 'get_exopite_sof_option' ) ) {
